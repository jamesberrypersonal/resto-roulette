...
- **`src/tui/`** — `ratatui`/`crossterm` interactive TUI split into submodules. `mod.rs` exports `run(buckets, initial_selection, explore)` which sets up raw mode and the alternate screen, then dispatches draw/key events to the active view module. Terminal is always restored on exit, even on error.
