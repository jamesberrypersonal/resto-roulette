...
cargo build --release -p resto-roulette-cli  # release build (CLI binary)
cargo check --workspace            # fast type-check without compiling (preferred during dev)
cargo test --workspace             # all unit + integration tests
cargo test <name>                  # run a single test by name substring
cargo fmt --all                    # format code
cargo fmt --check --all            # check formatting (used in CI)
cargo clippy --workspace -- -D warnings  # lint (all warnings are errors in CI)
```
...
### Core crate (`crates/resto-roulette-core/src/`)

- **`pipeline.rs`** — `run(inputs: &PipelineInputs, cache: &Cache) -> Result<Buckets>`. Orchestrates: parse → enrich (lazy, Places API) → filter closed → filter cuisine → fetch travel times (`buffer_unordered(10)`) → bucket. Stderr messages from the original `main.rs` are now `tracing::info!`/`warn!`.
- **`parse/`** — Three parser paths dispatched by file extension and CSV headers:
...
- **`places/`** — Google Places API (New) integration. Only active when `--open-now` or `--cuisine` (or `exclude_cuisines`) is set (lazy enrichment).
...
### CLI crate (`crates/resto-roulette-cli/src/`)

- **`main.rs`** — CLI entry point: tracing init → `Cli::parse()` → `config::load` → `Cache::open` + evict → build `PipelineInputs` → `pipeline::run` → empty-buckets check → TUI vs `display::render`.
- **`config.rs`** — Resolves configuration precedence: `--flag` > env var > `~/.resto-roulette/config.toml`. Note: `--home` short flag is `-H` (not `-h`, which clap reserves for `--help`).
- **`tui/`** — `ratatui`/`crossterm` interactive TUI split into submodules. `mod.rs` exports `run(buckets, initial_selection, explore)` which sets up raw mode and the alternate screen, then dispatches draw/key events to the active view module. Terminal is always restored on exit, even on error.
  - `app.rs` — `App` struct (holds `&Buckets`, current `slots`, `selected`, and `mode`), `Mode` enum (`Slots`/`Browse(BrowseState)`/`Explorer(ExplorerState)`), state types, and the `Action` enum returned by view handlers.
  - `slots_view.rs` — The default three-slot view. `↑↓`/`jk` navigates between buckets; `r`/`R` re-roll; `Enter` accepts; `Tab`/`l`/`→` enters browse mode for the selected bucket; `e` enters explorer mode; `q`/`Esc` quits.
  - `browse_view.rs` — Per-bucket candidate list using `ratatui::widgets::List` + `ListState` for scrolling. `Enter` assigns the highlighted entry to the slot and returns to the slot view; `Esc` cancels.
  - `explorer_view.rs` — Full-screen explorer with bucket tabs, a live search bar (`/` to activate, any printable key appends, `Esc` clears), sort cycling (`s`: Name→Time→Cuisine), and a detail panel. `h`/`l`/`←`/`→` switch bucket tabs; `Enter` picks and returns to the slot view.
  - `widgets.rs` — Shared `entry_name_line` and `entry_detail_line` helpers used by browse and explorer views.
- **`display.rs`** — `pretty` (colored terminal) and `json` output formatters. Pretty output shows cuisine inline when available (e.g. `→ Hà (Vietnamese · 243 Rue De Bleury)`); omits the address entirely when it equals the restaurant name (shared-list CSV format). JSON output includes a `cuisines` array on every entry.
...
- **Cuisine taxonomy**: `crates/resto-roulette-core/src/places/cuisine.rs` maps Google Places types to normalized lowercase display names. The taxonomy covers ~60 types observed in practice — extend `display_name()` there to add more.
- **TUI launch condition**: the TUI activates when `stdout.is_terminal() && cfg.format == Pretty && (cfg.explore || cfg.reroll)`. `--explore` contributes alongside `reroll` — so `--explore` with `--one-shot` still launches the TUI. `--format json` and piped output always bypass the TUI and fall back to a single `display::render` call.
...
- Integration tests in `crates/resto-roulette-core/tests/` cover parsing, bucketing, and picker logic using fixture files.
- Test fixtures (`crates/resto-roulette-core/tests/fixtures/`) include `sample.geojson`, `sample.csv`, `sample_maps_export.csv`, `routes_response.json`, `places_text_search_response.json`, and `places_details_response.json`.
